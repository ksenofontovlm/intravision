...
const OrderItem: React.FC<OrderItemProps> = ({ item, orderId }) => {
  ...
  const handleQuantityChange = (newQuantity: number) => {
    if (newQuantity >= 1 && newQuantity <= item.quantity) { // Используем item.quantity вместо product.quantityInStock
      dispatch(updateCartItem({ orderId, item: { productId: item.productId, quantity: newQuantity } }));
    }
  };
    ...
    <div className="flex justify-between items-center border-b py-2">
      ...
      <div className="flex items-center space-x-2">
        ...
        <input
          ...
          className="w-16 text-center border rounded"
          min="1"
          max={item.quantity} // Ограничиваем максимум текущим количеством
        />
        <button
          onClick={() => handleQuantityChange(item.quantity + 1)}
          disabled={item.quantity >= item.quantity} // Ограничение по текущему количеству
          className="px-2 py-1 bg-gray-200 rounded"
        >
          +
        </button>
